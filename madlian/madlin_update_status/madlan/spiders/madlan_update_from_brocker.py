...
class MadlanUpdateFromBrokerScraper(ScrapingBeeSpider):
    name = "madlan_update_from_brocker"
    listing_ids = None

    def __init__(self, *args, **kwargs):
        super(MadlanUpdateFromBrokerScraper, self).__init__(*args, **kwargs)
        try:
    ...
    def parse_details(self, response, **kwargs):
        ...
        if data_id not in response.url:
            ...
            from_broker = True if len(response.css('div[data-auto="secondary_address"]  div[data-auto^="agent-tag"]::text').get(
                '').strip()) > 0 else False
            street = row.get('Street')
            StreetNumber = row.get('StreetNumber')
            if (not street) or (not StreetNumber):
                from_broker = True
            row_copy['FromBroker'] = from_broker
            yield row_copy
